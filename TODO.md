...
- [ ] **Find & replace** — Search across the entire script by text, character name, or line type. Replace individually or in bulk. Standard in all competitors.
...
- [ ] **Cloud sync / account-based storage** — Save scripts to a server with user accounts so work persists across devices. Currently all storage is local JSON file download.
